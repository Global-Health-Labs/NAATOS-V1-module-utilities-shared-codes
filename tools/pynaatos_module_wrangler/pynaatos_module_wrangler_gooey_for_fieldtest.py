...
@Gooey(
    program_name="NAATOS Module Device Wrangler V0.4 (for field-test)",
    program_description="A tool to automatically download data from NAATOS module by clinical trial end-user.",
...
def main():
    ...
    group1.add_argument(
        ...
        gooey_options=dict(
            checkbox_label='Auto accept confirmed ID',
            visible = False
        ),
    ...
    group2b.add_argument(
        ...
        default=time.strftime('%Y%m%d_{:s}_autodl'.format('fieldtest')),
    );
    group2b.add_argument(
        "--datadl-deleteafter",
    ...
    group2b.add_argument(
        ...
        default=r'C:\Users\NAATOS\Documents\NAATOS\Modules'
    );
...
If this is blank, it will be calling uploadImage.bat in the current folder (current folder is {:s}).""".format(Path().absolute().as_posix()),
    ...
    args.device_reformat = not args.device_reformat;
