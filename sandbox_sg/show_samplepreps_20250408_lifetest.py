...
logreader.badfolders = ['_ignore','archive','LowerStallThreshold','IssueLogs'];
...
#%% contract unit names
df['unit'] = df['unit'].str.lower().str.replace(' ','')
...
for unit,dfunits in df.groupby('unit'):
    ...
    for run,dfrun in dfunits.groupby('run'):
        ...
        matchidx = dfrunevents[
                ...
                dfrunevents.str.startswith('Cycle 3 Started') |
                dfrunevents.str.startswith('Cycle 4 Stopped.')
            ].index;
        try:
...
def mkfig_lifetestsummary_medians():
    ...
    dfmedians = dflifetest.groupby(['run_number','run','unit','expname'])['MotorPWM'] \
        .median().reset_index();
    figtmp = px.scatter(dfmedians,
                x='run_number',y='MotorPWM',color='unit',symbol='expname',
                hover_data='run',category_orders={'unit':catorders},
                labels={'MotorPWM':'MotorPWM% median'}
            );
    figtmp.update_traces(marker_size=6);
    figtmp.update_layout(title='SP Lifetest MotorPWM Medians for Cycle 3 and Cycle 4')
    figtmp.update_xaxes(nticks=40);    # more ticks on x-axes
    return figtmp;
